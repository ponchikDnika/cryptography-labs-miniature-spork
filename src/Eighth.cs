...
public static class Eighth
{
    ...
    public static uint GetNthBit(uint num, int position) 
        => (uint)(num & (1 << position));

    public static uint SetNthBit(uint num, int bitPosition, uint value) 
        => (uint)((num & ~(1 << bitPosition)) | (value << bitPosition));
